  ...
  const handleSearch = () => {
    ...
    if (searchQuery.trim() === '') {
      ...
      const filteredResults = recipes.filter((recipe) => {
        ...
        const category = recipe.category && recipe.category.name ? recipe.category.name.toLowerCase() : ''; // Vérifier category.name
        ...
        console.log('Matching recipe:', recipe.title, isTitleMatch, isCategoryMatch, isSourceMatch);
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:5000/api/recipes')
      .then((response) => {
        ...
        const popular = recipesData.reduce((prev: IRecipe, current: IRecipe) => {
    ...
    <div className="recipes-page">
      ...
      {popularRecipe && (
        <div className="container-popular-recipe">
            <img className="title-popular-recipe" src="/img/mostpopular.png" alt="Most popular" />
          <div className="popular-recipe">
            <img src={`/img/${popularRecipe.image}`} alt={popularRecipe.title} />
            ...
            <div className="popular-score">
              {Array.from({ length: popularRecipe.popularity }, (_, index) => (
                <span className="popular-stars" key={index}>*</span>
              ))}
            </div>
