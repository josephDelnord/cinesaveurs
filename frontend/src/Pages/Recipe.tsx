...
const Recipe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      myAxiosInstance.get(`/api/recipes/${id}`)
        .then(response => {
      ...
      myAxiosInstance.get(`/api/recipes/${id}/scores`)
        .then(response => {
          console.log("Scores reçus:", response.data); // Vérifier les scores reçus
          const scoresData = response.data;
          setScores(scoresData);
          const average = scoresData.reduce((sum: number, score: number) => sum + score, 0) / scoresData.length;
  ...
  const handleSubmitComment = () => {
    ...
    myAxiosInstance
      ...
      .then((response) => {
        ...
        const updatedAverage = updatedScores.reduce((sum, score) => sum + score, 0) / updatedScores.length;
